...
(defun tiling-sizes (n band ctx element-n-byte)
  ...
  (with-inlined-foreign-funcall-mode
    ...
    (let* ((linesize (apply #'max *L1-Cache*)) ;; Byte
	   (size     (floor linesize element-n-byte))
	   (tilesize   (floor (min linesize (expt size (/ n)))))
...
(defun shift-band-zero (band ctx)
  ...
  (with-inlined-foreign-funcall-mode
    (let* ((domain           (%"isl_schedule_node_get_domain":pointer       :pointer band))
	   (partial-schedule (%"isl_schedule_node_band_get_partial_schedule":pointer :pointer band))
	   (mupa (%"isl_multi_union_pw_aff_intersect_domain":pointer
		   :pointer
		   (%"isl_multi_union_pw_aff_copy":pointer    :pointer partial-schedule)
		   :pointer
		   (%"isl_union_set_copy":pointer             :pointer domain)))
	   (n         (%"isl_multi_union_pw_aff_size":int     :pointer mupa))
	   (multi-val (%"isl_multi_union_pw_aff_min_multi_val":pointer :pointer mupa)))
      (loop for i upfrom 0 below n
	    for v = (%"isl_multi_val_get_val":pointer  :pointer multi-val :int i)
	    if (%"isl_val_is_neginfty":boolean        :pointer v) do ;; <- Is it legal???????
      ...
      (loop named find-tiling
	    while (> (%"isl_schedule_node_n_children":int   :pointer node) 0) do
	      (loop for i fixnum upfrom 0 below (%"isl_schedule_node_n_children":int  :pointer node)
			...
			(setf next-nodes (if next-nodes (append next-nodes (list child)) (list child)))
			(return-from find-tiling))
		    else do
		      (setf next-nodes (if next-nodes (append next-nodes (list band)) (list band))))
